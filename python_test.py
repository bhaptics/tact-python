...
async def main():
    ...
    while True:
        ...
        if user_input == 'q':
            ...
            request_id = await bhaptics_python.play_loop(event_name, 5.0, 1000.0, 0.0, 0.0, 1000, 10)
            print(f"{event_name} play_loop request_id {request_id}")
        elif user_input == 'result':
            await bhaptics_python.play_without_result(event_name, 0, 5.0, 1000.0, 0.0, 0.0)
            print(f"{event_name} play_without_result")
        elif user_input == 'pp':
            request_id = await bhaptics_python.play_position(event_name, 2)
            print(f"{event_name} play_position request_id {request_id}")
        elif user_input == 'ppp':
            request_id = await bhaptics_python.play_position_with_parameter(event_name, 0, 5.0, 1000.0, 0.0, 0.0)
